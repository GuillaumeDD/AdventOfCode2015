...
figure out which Aunt Sue (which you conveniently number 1 to 
500, for sanity) gave you the gift. You open the present and, 
as luck would have it, good ol' Aunt Sue got you a My First Crime
 Scene Analysis Machine! Just what you wanted. Or needed, as the 
 case may be.
...
detect a few specific compounds in a given sample, as well as how
 many distinct kinds of those compounds there are. According to the
  instructions, these are what the MFCSAM can detect:
    ...
    perfumes, which is handy, since many of your Aunts Sue wear a 
    few kinds.
 ...
 */
  case class Aunt(n: Int, info: Map[String, Int])

  def matchingAunt(a: Aunt): Boolean = {
  ...
  // Input
  def strToInfo(line: String): Aunt = {
    val regex = """Sue ([0-9]+): ([A-Za-z]+): ([0-9]+), ([A-Za-z]+): ([0-9]+), ([A-Za-z]+): ([0-9]+)""".r
    line match {
      case regex(n, info1, n1, info2, n2, info3, n3) =>
        val infos = Map(
          info1 -> n1.toInt,
          info2 -> n2.toInt,
          info3 -> n3.toInt)
  ...
  val aunts = input.filter { matchingAunt(_) }
  println(s"The number of the Sue is: ${aunts(0).n}")
