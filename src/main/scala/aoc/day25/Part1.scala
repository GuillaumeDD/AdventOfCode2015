 ...
 */
  def lineToRowColumnPair(line: String): (Int, Int) = {
    val regex = "To continue, please consult the code grid in the manual.  Enter the code at row ([0-9]+), column ([0-9]+).".r
    line match {
      case regex(row, column) =>
        (row.toInt, column.toInt)
    }
  }
  type Code = BigInt
  def nextCode(code: Code): Code =
  ...
  def codeAt(
    ...
    val j = column + gap
    val n = ((j + 1) * j) / 2 - gap
    (1 to n - 1).foldLeft(BigInt(20151125))((acc, _) => nextCode(acc))
  }

  val (row, column) = IO.getLines(lineToRowColumnPair).head
  println(s"You give the following code to the machine: ${codeAt(row, column)}")
