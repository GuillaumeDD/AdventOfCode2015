...
Your puzzle input is 34000000.
  ...
  def findMin(
    ...
    var i = 1
    var result = -1
    while (!found && i <= maxValue / nbPresent) {
      if (m(i) >= maxValue) {
        found = true
        result = i
      }
  ...
  def computeHouse(max: Int): Int = {
    val house = mutable.Map[Int, Int]()
    for {
      // For every Elf
      iElf <- 1 to max / 10
      // Computation of the number of delivered houses
      jHouse <- iElf to max / 10 by iElf
    } {
      if (house.contains(jHouse)) {
        house(jHouse) = house(jHouse) + iElf * 10
      } else {
        house(jHouse) = iElf * 10
      }
    }
    findMin(house, max, 10)
  }

  val input = IO.getLines(_.toInt).head

  val result = computeHouse(input)
  println(s"The lowest house number of the house to get at least as many presents as $input is: $result")
}