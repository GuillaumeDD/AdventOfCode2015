...
values (0 + 3 + 7 + 1 = 11) is 23 - 11 = 12.
   */
  def numberOfCharacterInMemory(s: String): Int =
    s.length()

  def numberOfCharacterInLiteral(s: String): Int =
    s.replaceAll("""(\\"|\\\\)""", "Z")
      .replaceAll("""\\x[0-9a-fA-F]{1,2}""", "Z") // HEXADECIMAL
      .length - 2

  def difference(s: String): Int =
    numberOfCharacterInMemory(s) - numberOfCharacterInLiteral(s)

  def computeSpace(strs: List[String]): Int =
    strs.map(difference(_)).sum
