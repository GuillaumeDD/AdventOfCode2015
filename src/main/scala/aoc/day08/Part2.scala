...
example, for the strings above, the total encoded length (6 + 9 + 16 + 11 = 42) minus
the characters in the original code representation (23, just like in the first part 
of this puzzle) is 42 - 23 = 19.
   */
  def count(str: String): Int =
    {
      def countSubstring(s: String, substr: String) =
        substr.r.findAllMatchIn(s).length
      /*
       * Count the number of \ and the number of ", and add 2 for the starting and ending "
       *  
       */
      countSubstring(str, """\\""") + countSubstring(str, """"""") + 2
    }

  def computeSpace(strs: List[String]): Int =
    strs.map(count(_)).sum

  val input = IO.getLines()
  println(s"Difference: ${computeSpace(input)}")  
}